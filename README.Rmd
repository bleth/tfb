...
**PLEASE NOTE!** Currently, `tfb` solves its optimization problem with [`RMosek`](https://cran.r-project.org/web/packages/Rmosek/index.html), a package requiring additional installation steps. See [here](https://docs.mosek.com/latest/rmosek/install-interface.html) for a guide to installation. [`RMosek`](https://cran.r-project.org/web/packages/Rmosek/index.html) can be used for free with an educational license.
...
1. The type of initial `fit` to use, representing what you think a reasonable representation of the relationship between `y` and `X` is. Currently, `tfb` supports ordinary least squares (`"ols"`), kernel-regularized least squares (`"krls"`), elastic net regularization (`"elasticnet"`), bayesian additive regression trees (`"bart"`), and a custom/manually-specified fit (`"custom"`). Kernel-regularized least squares is implemented by the [`KRLS`](https://cran.r-project.org/web/packages/KRLS/index.html) package, elastic net regression is implemented by the [`glmnet`](https://cran.r-project.org/web/packages/glmnet/index.html) package, and bayesian additive regression trees are implemented by the [`BART`](https://cran.r-project.org/web/packages/BART/index.html) package. If using a custom fit, then one must manually specify the coefficient vectors `beta_c` and/or `beta_t` (or the prediction vectors `fhat_c` and/or `fhat_t`), and the associated variance matrices `Vhat_c` and/or `Vhat_t` -- see `help(tfb)` for more information.
...
TFB includes 4 sample datasets for the user. They have been created via data generating process (DGP). For example, [DGP 1](https://github.com/bleth/tfb/blob/main/data-raw/tfb_sampledat1.R) was used to create the first dataset. We'll use the first dataset for our examples:
...
What else can `tfb` tell us? Its output is a nested list, where we can find summary statistics as well as raw information about the optimization. For example:
```{r}
# the final estimate
out$estimate
...
cbind(w, X, d, y)[1:10, ]
```
TFB's estimator is a weighted difference in means (WDIM), so naturally the weights can be used to manually calculate its estimates, if desired. The weights may also be investigated for trends in covariate importance. Estimating the ATC with TFB involves weighting the treatment set, so we can see that all the control units have the default weight of 1.

We can also inspect the model used for TFB, by looking at its coefficients and predicted values:
```{r}
# coefficients
out$betas
...
```
These outputs can be used for detailed work involving the predictive models fit as a part of TFB.


### Sample Splitting

TFB's weights rely out outcome information. However, if desired, one can implement sample splitting to decorrelate each weight with its associated outcome. To implement sample splitting, one can increase the number of `folds` (by default, `folds=1`) like so:
```{r}
...
names(out_ss)
```
If sample splitting is used, then the output contains a `final` list, which holds the final estimate, variance, confidence interval, and p-value:
```{r}
out_ss$final
```
The results for each sample-split iteration are split across the `sample_i` lists. Here we only have one sample-split iteration. However, one can increase the number of sample-split iterations by increasing the `samples` argument (by default, `samples=1`). Each `sample_i` list contains summary statistics for the sample-split iteration, which will be identical to those above in this case because only one sample has been taken:
```{r}
out_ss$sample_1$final$estimate
```
However, this lower level `sample_1` list also includes TFB's weights for that sample-split iteration:
```{r}
# retrieving weights
...
cbind(w, X, d, y)[1:10, ]
```
Again, because we are estimating the ATC here, we see that all the control units have the default weight of 1. The rest of the output within a sample-split iteration is broken out by sample-split fold:
```{r}
...
```
One can see that the WDIMs within each fold are averaged to determine the final estimate within the sample-split iteration:
```{r}
# Final estimate
...
Burton-Heckman, K. (2023). `tfb`*: a Package for Targeted Function Balancing* [Undergraduate thesis, Reed College]. Reed College Electronic Theses. [https://rdc.reed.edu/c/etheses/home/](https://rdc.reed.edu/c/etheses/home/)

Wainstein, L. & Bai, H. (2025). Targeted Function Balancing. *arXiv preprint arXiv:2203.12179*.
