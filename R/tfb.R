...
#' * `ev_approx`: Whether to abbreviate calculation of covariance matrices using [`RSpectra`](https://cran.r-project.org/web/packages/RSpectra/index.html), for when computation of the covariance matrix would be particularly taxing (large p). A logical. Default is `False.`
#' * `ev_n`: How many eigenpairs [`RSpectra`](https://cran.r-project.org/web/packages/RSpectra/index.html) should calculate. The user is recommended to analyze a complete sample eigenmatrix first, to look for a common breakpoint such that the calculated eigenvalues still account for most all of the total explained variance. If the number is too big with KRLS a full decomposition will be performed instead. An integer. The default is typically `5`.
